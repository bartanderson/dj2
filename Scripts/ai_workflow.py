...
def local_analyze_command():
    """Analyze with local AI, with auto-start and graceful fallback."""
    parser = argparse.ArgumentParser()
    parser.add_argument('--topic', required=True)
    parser.add_argument('--model', default='llama3.2:3b')
    args = parser.parse_args(sys.argv[2:])
    
    # Get client with auto-start enabled
    client = get_ollama_client(model=args.model, auto_start=True)
    
    # Build context
    context = ContextManager()
    context_package = context.build_context_for_ollama(args.topic)
    
    # Construct prompt
    prompt = f"""Development Analysis: {args.topic}
...
Format: Concise, 5 lines max."""
    ...
    try:
        ...
        print(f"\nAnalysis ({response.tokens_per_second:.1f} tokens/sec):")
        print("-" * 40)
        print(response_text)
        print("-" * 40)
        
        # Save to file
        output_path = AI_CONTEXT / "session" / "local_analysis.txt"
        ...
        print("   Tried to auto-start but failed")
