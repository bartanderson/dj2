...
class ContextManager:
    ...
    def load_system_docs(self) -> dict:
        ...
        if playbook_file.exists():
            ...
            docs["playbook"] = self.clean_ascii(content[:5000]) + "\n...[truncated]"
            self._vlog(f"Loaded development_playbook.md: {len(content)} chars (truncated to 5000)")
        else:
    ...
    def extract_code(self, query: str, max_lines: int = 150) -> dict:
        """Smart code extraction using ai.py search"""
        # Clean the query: remove common words, take first 2-3 meaningful words
        words = query.lower().split()
        common_words = {'continue', 'the', 'and', 'or', 'for', 'to', 'with', 'on', 'at', 'from', 'by', 'about', 'as', 'of', 'in', 'is', 'it', 'that', 'this', 'be', 'are', 'was', 'were', 'will', 'would', 'could', 'should', 'has', 'have', 'had', 'does', 'did', 'do', 'a', 'an', 'the'}
        key_terms = [word for word in words if word not in common_words][:3]
        
        if key_terms:
            search_query = ' '.join(key_terms)
        else:
            search_query = query[:50]  # Fallback
        self._vlog(f"Searching code for query: '{query}'")
        print(f"[SEARCH] Searching code for: {query}")
        
        try:
            cmd = [sys.executable, "ai.py", "search", query, "--limit", "5"]
            self._vlog(f"Running: {' '.join(cmd)}")
            print(f"[DEBUG] Running command: {' '.join(cmd)}")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)

            print(f"[DEBUG] Command output (first 500 chars):\n{result.stdout[:500]}")
            print(f"[DEBUG] Command error (if any): {result.stderr}")
            
            files = []
            for line in result.stdout.split('\n'):
                if '.py' in line and '.pyc' not in line:
                    match = re.search(r'([a-zA-Z_][\w/\\]*\.py)', line)
                    if match:
                        files.append(match.group(1))
            
            if not files:
                self._vlog("No Python files found via search")
                return {"strategy": "search", "context": "No Python files found", "files": 0}
            
            self._vlog(f"Found {len(files)} files: {', '.join(files[:3])}")
            
            context_parts = []
            for filepath_str in files[:3]:
                ...
                if filepath.exists():
                    try:
                        ...
                        for i, line in enumerate(lines):
                            if re.search(rf'(class|def).*{re.escape(query.split()[0])}', line, re.IGNORECASE):
                                end = min(len(lines), i + 30)
                                snippet = ''.join(lines[i:end])
                                context_parts.append(f"=== {filepath_str} ===\n{snippet}")
                                found = True
                                self._vlog(f"Found match in {filepath_str} at line {i}")
                                break
                        
                        if not found:
                            context_parts.append(f"=== {filepath_str} (preview) ===\n{''.join(lines[:20])}")
                            self._vlog(f"No specific match in {filepath_str}, showing preview")
                            
                    except Exception as e:
                        context_parts.append(f"=== {filepath_str} ===\n[Error: {e}]")
                        self._vlog(f"Error reading {filepath_str}: {e}")
                
                if sum(len(c) for c in context_parts) > max_lines * 50:
                    self._vlog("Reached max context size limit")
                    break
            
            return {
                "strategy": "search",
                "files": len(files),
                "context": "\n\n".join(context_parts) if context_parts else "No code extracted"
            }
            
        except Exception as e:
            self._vlog(f"Search failed: {e}")
            return {"strategy": "error", "context": f"Search failed: {e}", "files": 0}
    ...
    def build_context(self, query: str, include_files: bool = True) -> str:
        ...
        parts = [
            ...
            code_info.get('context', 'No code')[:2000],
        ]
        ...
        if self.verbose:
            ...
            self._vlog(f"  - Code: {len(code_info.get('context', ''))} chars")
    ...
    def build_package(self, query: str, target: str = "deepseek", max_tokens: int = 4000) -> dict:
        """
...
{docs.get('ai_contract', '[Missing]')[:1500]}

[TOOL] AVAILABLE TOOLS
{docs.get('working_tools', docs.get('tools', 'None'))}

[FOCUS] CURRENT SYSTEM FOCUS
{docs.get('current_focus', docs.get('status', 'None'))}
...
Tools: {docs.get('working_tools', docs.get('tools', 'None'))[:200]}
Violations: {violations[:500]}
Code: {code_info.get('context', 'None')[:1000]}"""
        ...
        self._vlog(f"Package built: {token_estimate} tokens, {code_info.get('files', 0)} files")
    ...
    def send_to_deepseek(self, context: str, keep_open: bool = False) -> str:
        try:
            ...
            from bridge.deepseek_bridge import DeepSeekBridge
