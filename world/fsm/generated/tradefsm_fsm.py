...
class TradeFSM(StateMachine):
    ...
    confirm = (
                awaiting.to(completed, on=[get_action('') for act in ["execute_buy"]])
 |                 countering.to(completed, on=[get_action('') for act in ["execute_buy"]])
    )
    offer = (
                awaiting.to(countering, cond=get_guard('price_too_low'), on=[get_action('') for act in ["update_price"]])
 |                 countering.to(countering, cond=get_guard('price_too_low'), on=[get_action('') for act in ["update_price"]])
 |                 awaiting.to(completed, cond=get_guard('price_acceptable'), on=[get_action('') for act in ["execute_buy"]])
 |                 countering.to(completed, cond=get_guard('price_acceptable'), on=[get_action('') for act in ["execute_buy"]])
    )
    cancel = (
 ...
 |                 countering.to(completed)
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        cls._state_meta = {
            'awaiting': {
                'prompt': '{item_name} costs {current_price} gp. Say 'yes' or offer a price.',
            },
            'countering': {
                'prompt': 'Offer too low. I want {current_price} gp. Say 'yes' or offer again.',
            },
            'completed': {
                'prompt': 'You bought {item_name} for {price} gp.',
            },
        }