...
class DungeonGenerator:
    ...
    def generate_png(self):
        ...
        def draw_door(draw, x, y, cell_size, door_type, orientation, is_legend=False):
            ...
            if door_type in ['door', 'lock', 'trap', 'secret']:
                if orientation == 'horizontal':
                    draw.rectangle([
                        x + (cell_size - door_width)//2, y + arch_height,
                        x + (cell_size + door_width)//2, y + cell_size - arch_height
        ...
        items = [
            ...
            {'name': 'Secret Door', 'draw': lambda d, x, y: draw_door(d, x, y, icon_size, 'secret', 'horizontal', True)},
            {'name': 'Secret Door(debug)', 'draw': lambda d, x, y: draw_door(d, x, y, icon_size, 'secret', 'horizontal', True)},
