...
// Update the map transform function
function updateMapTransform() {
    const transform = `translate(${panX}px, ${panY}px) scale(${scale})`;
    terrainCanvas.style.transform = transform;
}

// Add this function to safely set up event listeners
function safeAddEventListener(selector, event, handler) {
    const element = document.querySelector(selector);
    if (element) {
        element.addEventListener(event, handler);
    } else {
        console.warn(`Element ${selector} not found for event listener`);
    }
}
...
function drawLocations(ctx, locations) {
    locations.forEach(loc => {
        ...
        if (!window.worldState.locations) window.worldState.locations = [];
...
// Add event listeners to the canvas for location interactions
function setupCanvasInteractions() {
    ...
    terrainCanvas.addEventListener('mousemove', function(e) {
        ...
        if (hoveredLocation) {
            ...
            showLocationPreview(hoveredLocation.data, e.clientX, e.clientY);
        } else {
...
window.addEventListener('load', function() {
    if (window.worldState && window.worldState.worldMap) {
        renderWorldMap(window.worldState.worldMap);
        // Call this after the initial render
...
window.addEventListener('load', () => {
    ...
    // Update the zoom and pan event handlers to redraw the map
    ...
    // Add a function to redraw the entire map
    function redrawMap() {
        if (window.worldState && window.worldState.worldMap) {
            renderWorldMap(window.worldState.worldMap);
        }
...
// Update the setupCanvasInteractions function to use the singleton
function setupCanvasInteractions() {
    const terrainCanvas = document.getElementById('terrain-canvas');
