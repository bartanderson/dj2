...
12b. **Intent Layer - semantic summaries + knowledge artifacts (DESIGN.md
    section 3):** two sub-layers designed 2026-06-19, nothing built.
    - Sub-layer A (AI-generated semantic summaries): per-file/module/subsystem
      descriptions generated by AI reading the source, stored with a
      source_hash for staleness detection. Replaces the keyword-heuristic
      Role view signal with something grounded. Generate lazily on first
      query, not at ingestion time. `semantic_summaries` table.
    - Sub-layer B (knowledge artifacts): durable findings, strategy
      decisions, and confirmed facts produced during sessions - stored
      deliberately (not auto-captured) via an explicit "keep this" signal.
      `knowledge_artifacts` table with provenance field (human-confirmed
      outranks ai-generated). These answer questions the structural layer
      cannot answer at all.
    - Sequencing: after `inspection/explain_file.py` is wired (that wiring
      reveals where heuristics break), and alongside the Agent Capability
      Layer task.md mechanism (task.md is a natural first consumer of the
      artifact store). See DESIGN.md section 3 for full schema and
      staleness model reasoning. Classification: **signal quality upgrade**
      (not make-work, not a new query type - it makes existing answers
      materially better and makes intent-level questions answerable at all).
