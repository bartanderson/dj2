...
Executes one tool call per turn to avoid dependency issues.
"""
import json
import re
import sys
    ...
    for token in args_list:
        ...
        if '=' in token:
            ...
            if key in param_names:
                ...
                # Unexpected key – ignore or store? For now, ignore.
                pass
        else:
            # If any token is not a keyword, we'll fall back to positional for all
            # So break out and handle positional
            args_dict = {}
            break
...
def main():
    ...
    for turn in range(args.max_turns):
        ...
        if tool_calls:
            ...
            # If this was call_deepseek, return the result as final
            if tool_name == "call_deepseek":
                if args.output:
                    Path(args.output).write_text(result, encoding='utf-8')
                    print(f"✅ Test written to {args.output}")
                else:
                    print("\n" + "="*40)
                    print(result)
                    print("="*40)
        ...
        # No tool calls, check for final answer in triple backticks
        if "```" in text:
            match = re.search(r"```(?:\w+)?\n(.*?)\n```", text, re.DOTALL)
            if match:
