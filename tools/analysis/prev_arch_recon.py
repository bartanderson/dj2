...
def validate_python_syntax(code: str) -> tuple[bool, str]:
    """Check if the code is syntactically valid Python. Returns (is_valid, error_message)."""
    try:
        ast.parse(code)
        return True, ""
...
def clean_ai_response(response: str) -> str:
    """Extract Python code from AI response and strip metadata."""
    import re
    # Try to extract code between triple backticks (with or without language spec)
    code_match = re.search(r'```(?:python)?\n(.*?)\n```', response, re.DOTALL)
    if code_match:
        response = code_match.group(1)
    else:
        # If no backticks, strip lines that look like metadata
        lines = response.splitlines()
        cleaned = []
        for line in lines:
            line = line.rstrip()
            # Skip common metadata lines
            if (line.startswith('Model:') or 
                line.startswith('Generated by') or
                line.startswith('Here is') or
                line.startswith('Here\'s') or
                line.startswith('```') or
                (line.strip() and not line.strip().startswith('#') and not any(line.strip().startswith(kw) for kw in ['def ', 'class ', 'import ', 'from ', '@', 'assert', 'mock', 'patch']))):
                # If line doesn't look like Python, skip it (crude heuristic)
                # Actually, better to just trust the user's heuristic; but we'll keep it simple.
                pass
            else:
                cleaned.append(line)
        response = '\n'.join(cleaned)
...
def auto_fix_braces(code: str) -> str:
    """Add missing closing braces/parentheses, trying to place them intelligently."""
    lines = code.splitlines(keepends=True)
    
    # Track open/close counts per line to find likely insertion points
    open_counts = {'{': 0, '(': 0, '[': 0}
    close_counts = {'}': 0, ')': 0, ']': 0}
    ...
    # Calculate deficits
    deficits = {
        '}': open_counts['{'] - close_counts['}'],
        ')': open_counts['('] - close_counts[')'],
        ']': open_counts['['] - close_counts[']']
    }
    
    # If all balanced, return as-is
    if all(d <= 0 for d in deficits.values()):
        return code
    
    # Find lines that opened but didn't close (simple heuristic: last non-empty lines)
    # Work backwards to find where to insert closing braces
    result_lines = list(lines)
    
    # Add missing closing braces at the very end (conservative approach)
    # but grouped by type: first ), then ], then }
    suffix = ''
    suffix += ')' * max(0, deficits[')'])
    suffix += ']' * max(0, deficits[']'])
    suffix += '}' * max(0, deficits['}'])
    
    if suffix:
        # Add before any trailing whitespace/newlines
        while result_lines and result_lines[-1].strip() == '':
            suffix = result_lines.pop() + suffix
        
        if result_lines:
            # Append to last non-empty line if it doesn't end with newline
            if not result_lines[-1].endswith('\n'):
                result_lines[-1] = result_lines[-1] + suffix + '\n'
            else:
...
def fix_syntax_heuristics(code: str) -> str:
    """Apply all syntax-fixing heuristics in order."""
    lines = code.splitlines()
    fixed = []
    paren_stack = []
    
    # Phase 1: Fix per-line issues (from auto_fix_common_errors)
    for i, line in enumerate(lines):
        # Count parentheses to track balance
        for ch in line:
            if ch == '(':
        ...
        # If at end of file and stack not empty, add missing closing parentheses
        if i == len(lines)-1 and paren_stack:
            line += ')' * len(paren_stack)
        fixed.append(line)
    
    code = '\n'.join(fixed)
    
    # Phase 2: Fix AI-specific mistakes (from fix_common_ai_mistakes)
    import re
    
    # Fix assignment inside function calls: func(obj.attr = value) -> func(value)
    assignment_in_call = re.compile(r'(\w+\s*\([^)]*?)\b(\w+)\.(\w+)\s*=\s*([^,)]+)([),])')
    ...
    code = assignment_in_call.sub(replace_assignment, code)
    
    return code

def get_context_lines(code: str, error_line: int, context: int = 3) -> str:
    """Extract lines around the error for better context."""
    lines = code.splitlines()
    start = max(0, error_line - context - 1)  # -1 because error_line is 1-indexed
...
def archive_response_file(response_path: Path, attempt: int = 0) -> Path:
    """Copy response file with timestamp and attempt number for retention."""
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    stem = response_path.stem.replace('deepseek_response', f'deepseek_response_{timestamp}_attempt{attempt}')
    archive_name = f"{stem}.txt"
    archive_path = response_path.parent / archive_name
    
    try:
        import shutil
        shutil.copy2(response_path, archive_path)
        return archive_path
    except Exception as e:
        # If copy fails, return original but print warning
        print(f"⚠️  Could not archive response: {e}", file=sys.stderr)
...
def consult_mode(intent: str, db_path: Path, project_root: Optional[Path] = None,
    ...
    if response_files:
        ...
        # Archive it before processing
        archived = archive_response_file(latest, attempt=0)
        print(f"💾 Archived initial response: {archived.name}")
...
def generate_test(intent: str, db_path: Path, categories_path: Optional[str] = None,
                  ...
                  verbose: bool = False):
    import sys
    # ---------- helper: validate imports and patch targets ----------
    def validate_test_imports(code: str) -> list:
        """Check all import statements and @patch targets against scout DB.
        Returns list of issues (empty if all good)."""
        from tools.analysis.test_templates import TestValidator
        import re

        # Known standard library modules to skip (add more as needed)
        STDLIB_MODULES = {
            'unittest', 'json', 're', 'os', 'sys', 'datetime', 'time',
            'collections', 'itertools', 'functools', 'math', 'random',
            'typing', 'pathlib', 'contextlib', 'io', 'abc', 'copy',
            'enum', 'inspect', 'warnings', 'traceback', 'logging'
        }

        validator = TestValidator(db_path)
        issues = []
            ...
            if line.startswith('from ') or line.startswith('import '):
                ...
                if match:
                    ...
                    if module in STDLIB_MODULES:
                        continue
                    valid, msg = validator.validate_mock_target(module)
                    if not valid:
                        issues.append(f"Import '{module}' not found: {msg}")
                # Could also check "import X", but skip for simplicity

        # Find @patch decorators
        patch_pattern = r'@patch\([\'"]([^\'"]+)[\'"]\)'
        for match in re.finditer(patch_pattern, code):
            target = match.group(1)
            # Skip if target starts with a stdlib module
            top_module = target.split('.')[0]
            if top_module in STDLIB_MODULES:
                continue
            valid, msg = validator.validate_mock_target(target)
            if not valid:
                issues.append(f"Patch target '{target}' not found: {msg}")
    ...
    # Determine target_file for ALL modes (not just template mode)
    top_files = _get_top_files_for_intent(intent, db_path, categories_path, max_files=1, verbose=verbose)
    target_file = top_files[0][0] if top_files else None
    ...
    if use_template:
        try:
            ...
            if top_files:
                ...
                if contracts:
                    # Get list of existing top-level modules to help AI avoid hallucinations
                    conn_modules = sqlite3.connect(str(db_path))
                    rows = conn_modules.execute("""
                        SELECT DISTINCT substr(path, 1, instr(path, '/')-1) as module
                        FROM files
                        WHERE path LIKE '%/%'
                    """).fetchall()
                    module_list = ', '.join([r[0] for r in rows if r[0]])
                    conn_modules.close()
                    ...
                    if template:
                        # Build the specialized prompt, now passing module_list
                        prompt = lib.build_generation_prompt(target_file, contracts, template, module_list)
                        context_text = prompt
                    else:
...
"""
    ...
    for attempt in range(max_attempts):
        is_valid, error_msg = validate_python_syntax(response)
        if is_valid:
            break

        print(f"⚠️  Generated test has syntax error (attempt {attempt+1}/{max_attempts}): {error_msg}")
        if attempt == max_attempts - 1:
            print("❌ Maximum retries reached. Writing invalid test for manual review.")
            break
        ...
        if error_line:
            context_snippet = get_context_lines(response, error_line)
            correction_prompt = f"""The previous test for intent "{intent}" (target file: {target_file or 'unknown'}) has a syntax error at line {error_line}:
...
- Missing closing brace: add }} at end of line {error_line} or the line that opened it
...
"""
            ...
            correction_prompt = f"""The previous test for intent "{intent}" (target file: {target_file or 'unknown'}) has a syntax error:
...
"""
        ...
        with contextlib.redirect_stdout(f_correction):
            success = mgr.send(correction_package, target='deepseek', keep_open=False)
        if not success:
            print("❌ Failed to get correction from AI.", file=sys.stderr)
            break
        ...
        if response_files:
            latest = max(response_files, key=lambda p: p.stat().st_mtime)
            # Archive with attempt number
            archived = archive_response_file(latest, attempt=attempt + 1)
            if archived != latest:
                print(f"💾 Archived correction attempt {attempt + 1}: {archived.name}")
            response = latest.read_text(encoding='utf-8')
        else:
            print("⚠️  Could not locate AI response file for correction.", file=sys.stderr)
            break
    ...
    is_valid, error_msg = validate_python_syntax(response)
    if not is_valid:
        print(f"❌ Final test still has syntax error: {error_msg}")
        # List all archived responses for comparison
        session_dir = mgr.session_dir
        archives = sorted(session_dir.glob("deepseek_response_*_attempt*.txt"))
        if archives:
            print(f"\n📋 Response history ({len(archives)} attempts):")
            for i, archive in enumerate(archives, 1):
                print(f"   {i}. {archive.name}")
        print("   You will need to fix it manually.")
        print(f"\n💡 Compare attempts to see what changed:")
        print(f"   fc /L {archives[0]} {archives[-1]}" if len(archives) >= 2 else "")
    else:
        print("✅ Syntax check passed.")

    # Validate imports and patch targets
    import_issues = validate_test_imports(response)
    if import_issues:
        print("⚠️  Found invalid imports/patch targets:")
        for issue in import_issues[:5]:  # show first 5
            print(f"   - {issue}")
        # Optionally, you could trigger another correction round here
